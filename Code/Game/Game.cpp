...
// 	m_tangentSpaceMaterial->m_shader->SetDepth(COMPARE_LESS_EQUAL, true);
	...
	m_funkyTextShader = g_theRenderer->CreateOrGetShader("default_font.hlsl");
	//m_funkyTextShader = g_theRenderer->CreateOrGetShader("depth.hlsl");
...
//  	g_theRenderer->BindSampler(SAMPLE_MODEL_LINEAR);
	...
	m_tierOneFont = g_theRenderer->CreateOrGetBitmapFont("16x16-Dwarf.png", FONT_FIXED_WIDTH);
	m_tierTwoFont = g_theRenderer->CreateOrGetBitmapFont("SquirrelPix_MedBoldProp9_16x16.png", FONT_PROPORTIONAL);
	m_tierThreeFont_no_kerning = g_theRenderer->CreateOrGetBitmapFont("Lobster.png", FONT_PACKED);
	m_tierThreeFont_w_kerning = g_theRenderer->CreateOrGetBitmapFont("Arimo.png", FONT_PACKED);
}
...
void Game::Render() const
{
	...
	//font drawing	- t1
	const TextureView* font_texture = m_tierOneFont->GetTexture();
	g_theRenderer->BindTextureViewWithSampler(0U, font_texture, SAMPLE_MODE_POINT);
	
	CPUMesh font_mesh;
	m_tierOneFont->AddMeshForText2D(&font_mesh, Vec2(-23.5f, -5.0f), 1.0f, m_testSentence, Rgba::RED);
	m_lineMesh->CreateFromCPUMesh<Vertex_PCU>(font_mesh);
	g_theRenderer->DrawMesh(*m_lineMesh);
	
	// t2
	font_texture = m_tierTwoFont->GetTexture();
	g_theRenderer->BindTextureViewWithSampler(0U, font_texture, SAMPLE_MODE_POINT);
	
	font_mesh.Clear();
	m_tierTwoFont->AddMeshForText2D(&font_mesh, Vec2(-23.5f, -6.0f), 1.0f, m_testSentence, Rgba::RED);
	m_lineMesh->CreateFromCPUMesh<Vertex_PCU>(font_mesh);
	g_theRenderer->DrawMesh(*m_lineMesh);
	...
	// t3 without kernning data
	g_theRenderer->BindShader(m_funkyTextShader);
	
	font_texture = m_tierThreeFont_no_kerning->GetTexture();
	g_theRenderer->BindTextureViewWithSampler(0U, font_texture, SAMPLE_MODEL_LINEAR);

	font_mesh.Clear();
	m_tierThreeFont_no_kerning->AddMeshForText2D(&font_mesh, Vec2(-23.5f, -7.0f), 	1.0f, m_testSentence, Rgba::RED);
	m_lineMesh->CreateFromCPUMesh<Vertex_PCU>(font_mesh);
	g_theRenderer->DrawMesh(*m_lineMesh);

	// t3 with kernning data
	font_texture = m_tierThreeFont_w_kerning->GetTexture();
	g_theRenderer->BindTextureViewWithSampler(0U, font_texture, SAMPLE_MODEL_LINEAR);

	font_mesh.Clear();
	m_tierThreeFont_w_kerning->AddMeshForText2D(&font_mesh, Vec2(-23.5f, -8.0f), 1.0f, m_testSentence, Rgba::RED);
	m_lineMesh->CreateFromCPUMesh<Vertex_PCU>(font_mesh);
	g_theRenderer->DrawMesh(*m_lineMesh);
	...
	font_mesh.Clear();
	m_tierThreeFont_no_kerning->AddMeshForText2D(&font_mesh, Vec2(-23.5f, -9.0f), 1.0f, m_paragraph, Rgba::RED);
	m_lineMesh->CreateFromCPUMesh<Vertex_PCU>(font_mesh);
	g_theRenderer->DrawMesh(*m_lineMesh);
	...
	font_mesh.Clear();
	m_tierThreeFont_w_kerning->AddMeshForText2D(&font_mesh, Vec2(-23.5f, -10.0f), 1.0f, m_paragraph, Rgba::RED);
	m_lineMesh->CreateFromCPUMesh<Vertex_PCU>(font_mesh);
	g_theRenderer->DrawMesh(*m_lineMesh);
